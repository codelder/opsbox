    ...
    for (const kw of nonEmpty) {
      ...
      if (posRel !== -1) {
        ...
        if (
          bestPos == null ||
          pos < bestPos ||
          (pos === bestPos && kw.length > bestKw.length)
        ) {
          bestPos = pos;
          bestKw = kw;
        }
...
export default function JsonSearchPage() {
  ...
  return (
    <main className="container mx-auto max-w-7xl space-y-6 p-6">
      ...
      {error && (
        <div className="rounded-md border border-red-500 bg-red-950/50 px-3 py-2 text-red-300">
          {error}
        </div>
      )}
      ...
      <ScrollArea ref={areaRef}>
        <div className="space-y-6">
          {results.map((r, idx) => (
            <section
              key={idx}
              className="rounded-lg border border-zinc-700/60 bg-zinc-900 p-4"
            >
              <h2 className="font-mono text-sm break-all text-zinc-300">
          ...
          {results.length === 0 && !loading && !error && (
            <div className="text-zinc-400">
              No results yet. Submit a search to start streaming.
            </div>
          )}
        </div>
      </ScrollArea>
