...
type Row =
  | { kind: "header"; key: string; path: string; keywords: string[] }
  | {
      kind: "code";
      key: string;
      no: number;
...
function highlightWithMark(input: string, keywords: string[]): string {
  ...
  while (i < input.length) {
    ...
    for (const kw of ks) {
      ...
      if (
        p !== -1 &&
        (p < bestPos || (p === bestPos && kw.length > bestKw.length))
      ) {
        bestPos = p;
        bestKw = kw;
      }
...
function toRows(r: SearchJsonResult): Row[] {
  ...
  rows.push({
    kind: "header",
    key: `h:${r.path}:${crypto.randomUUID()}`,
    path: r.path,
    keywords: r.keywords,
  });
  r.chunks.forEach((ch, ci) => {
    ...
    if (ci + 1 < r.chunks.length)
      rows.push({ kind: "sep", key: `s:${r.path}:${ci}` });
  });
  return rows;
}
...
export default function Page() {
  ...
  const start = React.useCallback(
    async (keywords: string[], context: number) => {
      ...
      if (last) {
        try {
          queueRef.current.push(
            ...toRows(JSON.parse(last) as SearchJsonResult),
          );
          scheduleFlush();
        } catch {}
      }
    ...
    <main className="container mx-auto max-w-7xl space-y-6 p-6">
      <div className="flex items-center justify-between">
        ...
        <nav className="flex items-center gap-3 text-sm">
          ...
          <Link className="text-sky-400 hover:underline" href="/json">
            JSON page
          </Link>
          <span className="text-zinc-400">/</span>
      ...
      >
        <input
          name="q"
          className="flex-1 rounded-md border border-zinc-800 bg-zinc-900 px-3 py-2 outline-none focus:ring-2 focus:ring-sky-500"
        ...
        <input
          ...
          min={0}
          defaultValue={3}
          className="w-28 rounded-md border border-zinc-800 bg-zinc-900 px-3 py-2 outline-none focus:ring-2 focus:ring-sky-500"
        />
        <button className="rounded-md bg-sky-600 px-4 py-2 hover:bg-sky-500">
          Search
        </button>
        <button
          type="button"
          onClick={() => abortRef.current?.abort()}
          className="rounded-md bg-zinc-700 px-3 py-2 hover:bg-zinc-600"
        >
          Stop
        </button>
        <button
      ...
      <div
        ref={parentRef}
        className="h-[70vh] overflow-auto rounded-md border border-zinc-700/60 bg-zinc-900"
      >
        {(() => {
              ...
              {items.map((vi) => {
                ...
                return (
                  ...
                  >
                    {row.kind === "header" && (
                      <div className="border-b border-zinc-800 bg-zinc-900 px-3 py-2">
                        <div className="font-mono text-sm break-all text-zinc-300">
                          s3://test/codeler.tar.gz::{row.path}
                        </div>
                    ...
                    {row.kind === "code" && (
                      <div className="px-3 py-[2px] text-sm">
                        <div
                          className="grid min-w-0 items-start font-mono"
                          style={{ gridTemplateColumns: "48px 18px 1fr" }}
                        >
                          <span className="text-right text-zinc-500 tabular-nums select-none">
                            {String(row.no).padStart(6, " ")}
                          </span>
                          <span
                          ...
                          <span
                            ...
                            dangerouslySetInnerHTML={{
                              __html: highlightWithMark(row.text, row.keywords),
                            }}
                          />
                        </div>
                      </div>
                    ...
                    {row.kind === "sep" && (
                      <div className="px-3 py-1 text-sm text-zinc-500">…</div>
                    )}
                  </div>
                );
              })}
