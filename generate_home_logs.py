...
def generate_trace_filename():
    """生成trace目录下的数字文件名"""
    # 格式: HHMMSS0420250818XXXXXXX.log
    hour = random.randint(0, 23)
    minute = random.randint(0, 59)
    second = random.randint(0, 59)
    random_suffix = random.randint(1000000, 9999999)
    
    return f"{hour:02d}{minute:02d}{second:02d}0420250818{random_suffix}.log"
...
def create_directory_structure():
    ...
    for service in SERVICE_NAMES:
        ...
        if service not in ["bjbcbp-bjbbfecsbp", "bjbcbp-bjbfrhhz", "bjbcbp-gateway", "bjbbip-agent", 
            ...
            # 创建2025-08-18目录
            date_dir = trace_dir / "2025-08-18"
            date_dir.mkdir(exist_ok=True)
        
        # 为大部分服务创建2025-08-18目录
        if service not in ["bjbcbp-bjbbfecsbp", "bjbcbp-bjbfrhhz", "bjbcbp-bjlogs", "msk", 
            ...
            date_dir = service_dir / "2025-08-18"
            date_dir.mkdir(exist_ok=True)
...
def generate_files(base_dir):
    ...
    for service in SERVICE_NAMES:
        ...
        trace_dir = service_dir / "trace" / "2025-08-18"
        if trace_dir.exists():
            # 生成5-15个trace文件
