...
use opsbox_core::{
  database::{DatabaseConfig, init_pool},
  logging::{LogLevel, repository::LogConfigRepository},
};
use serde_json::json;
use std::sync::Once;
...
/// 创建测试路由
fn create_test_router(pool: opsbox_core::SqlitePool, log_dir: std::path::PathBuf) -> axum::Router {
...
async fn test_get_log_config_success() {
  ...
  repo
    .update_level("server", LogLevel::Info)
    .await
    .expect("初始化日志级别失败");
  repo.update_retention("server", 7).await.expect("初始化保留数量失败");
