...
impl OpsFileSystem for S3DiscoveryFileSystem {
  async fn metadata(&self, path: &OpsPath) -> io::Result<OpsMetadata> {
    ...
    Ok(OpsMetadata {
      name: if path_str == "/" { "s3_root".to_string() } else { path_str.to_string() },
  ...
  async fn read_dir(&self, path: &OpsPath) -> io::Result<Vec<OpsEntry>> {
    ...
    if profile_name.contains('/') {
       return Err(io::Error::new(io::ErrorKind::InvalidInput, "S3 Discovery only supports 2 levels (root and profile)"));
    }
    ...
    let entries = resp.buckets.unwrap_or_default().into_iter().map(|b| {
         ...
         let created = b.creation_date.map(|d| std::time::UNIX_EPOCH + std::time::Duration::from_secs(d.secs() as u64));
  ...
  async fn open_read(&self, _path: &OpsPath) -> io::Result<OpsRead> {
    Err(io::Error::new(io::ErrorKind::PermissionDenied, "Cannot read S3 root as file"))
  }
