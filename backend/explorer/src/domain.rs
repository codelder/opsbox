...
mod tests {
  ...
  fn test_resource_type_deserialization() {
    assert_eq!(
      serde_json::from_str::<ResourceType>("\"file\"").unwrap(),
      ResourceType::File
    );
    assert_eq!(
      serde_json::from_str::<ResourceType>("\"dir\"").unwrap(),
      ResourceType::Dir
    );
    assert_eq!(
      serde_json::from_str::<ResourceType>("\"linkfile\"").unwrap(),
      ResourceType::LinkFile
    );
    assert_eq!(
      serde_json::from_str::<ResourceType>("\"linkdir\"").unwrap(),
