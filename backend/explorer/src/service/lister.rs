...
impl ResourceLister {
    ...
    pub async fn list_archive(
        ...
        // 创建归档文件系统
        let archive_fs = ArchiveFileSystem::with_path(
            local_fs,
            archive_type,
    ...
    /// * `Err(String)` - 错误信息
    pub async fn download_local(&self, path: &Path) -> Result<(String, u64, Pin<Box<dyn AsyncRead + Send + Unpin>>), String> {
        if !path.exists() {
    ...
    pub async fn download_archive_entry(
        ...
        // 获取元数据
        let meta = archive_fs.metadata(&entry_resource_path)
            .await
            .map_err(|e| {
                let error_str = e.to_string();
        ...
        // 打开文件
        let reader = archive_fs.open_read(&entry_resource_path)
            .await
            .map_err(|e| {
                let error_str = e.to_string();
                if error_str.contains("numeric field did not have utf-8") {
    ...
    pub async fn list_archive_from_reader(
        ...
        // 使用临时文件列出归档内容
        self.list_archive_with_temp_file(&temp_path, archive_type, inner_path, temp_file).await
    }
    ...
    pub async fn download_archive_entry_from_reader(
        ...
        // 打开文件
        let dfs_reader = archive_fs.open_read(&entry_resource_path)
            .await
            .map_err(|e| format!("Failed to open file: {}", e))?;
...
mod tests {
    ...
    fn test_guess_mime_type() {
        assert_eq!(ResourceLister::guess_mime_type("test.txt"), Some("text/plain".to_string()));
        assert_eq!(ResourceLister::guess_mime_type("test.json"), Some("application/json".to_string()));
        assert_eq!(ResourceLister::guess_mime_type("test.log"), Some("text/plain".to_string()));
        assert_eq!(ResourceLister::guess_mime_type("test.png"), Some("image/png".to_string()));
    ...
    async fn test_list_tar_archive() {
        ...
        // 创建 tar 归档
        {
            ...
            header.set_size(5);
            header.set_cksum();
            builder.append_data(&mut header, "file1.txt", "hello".as_bytes()).unwrap();
    ...
    async fn test_list_tar_gz_archive() {
        ...
        // 创建 tar.gz 归档
        {
            ...
            header.set_size(5);
            header.set_cksum();
            builder.append_data(&mut header, "inner.log", "hello".as_bytes()).unwrap();

            builder.finish().unwrap();
        }

        let lister = ResourceLister::new();
        let entries = lister.list_archive(&tar_gz_path, ArchiveType::TarGz, None).await.unwrap();
