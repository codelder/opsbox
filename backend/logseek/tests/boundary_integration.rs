...
async fn test_mixed_encoding_search() {
  ...
  let utf8_bom_path = generator
    .create_file("utf8_bom.log", utf8_bom_content)
    .await
    .expect("创建UTF-8 BOM文件失败");

  // 创建UTF-8文件（无BOM）
  let utf8_content = "UTF-8 without BOM content\n测试内容\n2024-01-01 INFO Test message";
  let utf8_path = generator
    .create_file("utf8.log", utf8_content)
    .await
    .expect("创建UTF-8文件失败");
  ...
  let gbk_path = generator
    .create_file("gbk.log", gbk_content)
    .await
    .expect("创建GBK文件失败");
...
async fn test_large_file_boundary() {
  ...
  let large_file_path = generator
    .create_large_file("large.log", 10) // 10MB
    .await
    .expect("创建大文件失败");
