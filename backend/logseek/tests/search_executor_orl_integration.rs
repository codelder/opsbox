...
"#,
  ...
  planners::upsert_script(&pool, "test_orl_local", &script).await.unwrap();
  ...
  // 执行搜索
  let result = executor
    .search("app:test_orl_local UNIQUE_MARKER_123", "test-sid".to_string(), 1, None)
    .await;
...
"#,
  ...
  let result = executor
    .search(
      "app:test_relative_glob TARGET_LOG_CONTENT_789",
      "test-sid".to_string(),
      1,
      None,
    )
...
async fn test_create_entry_stream_with_orl() {
  ...
  assert!(entry_count >= 1, "Should have at least 1 entry, got {}", entry_count);
}
