...
use opsbox_test_common::{
  bench,
  performance::{PerformanceRunner, assert_reasonable_time},
};
use regex::Regex;
...
fn test_reasonable_time_assertion() {
  // 验证快速操作在合理时间内完成
  let result = assert_reasonable_time("快速计算", Duration::from_millis(100), || {
...
fn test_performance_examples() {
  ...
  println!();
  println!("⚡ bench宏示例:");
  let result = bench!("计算平方和", { (0..1000).map(|x| x * x).sum::<u32>() });
  println!("  结果: {}", result);
