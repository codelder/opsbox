...
async fn test_xss_detection() {
  ...
  for vector in security::xss::TEST_VECTORS {
    ...
    assert!(report.xss_detected, "Should detect XSS in: {}", vector);
...
async fn test_safe_input_handling() {
  ...
  for input in safe_inputs {
    ...
    assert!(report.passed(), "Safe input should pass security check: {}", input);
...
async fn test_boundary_inputs() {
  ...
  for (name, input) in boundary_inputs {
    ...
    // 这里主要确保不会崩溃
    println!(
      "✓ Processed boundary input '{}' (length: {}): passed={}",
      name,
      input.len(),
      report.passed()
    );
