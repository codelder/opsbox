...
use opsbox_core::dfs::{orl_parser::OrlParser, endpoint::Location};
use std::fs::File;
use std::io::Write;
use tar::Builder;
...
fn create_test_zip_archive(dir: &std::path::Path) -> std::path::PathBuf {
    ...
    zip.start_file::<_, ()>("logs/file1.log", FileOptions::default())
        .expect("Failed to start file");
    zip.write_all(b"log content 1\n")
        .expect("Failed to write content");

    zip.start_file::<_, ()>("logs/file2.log", FileOptions::default())
        .expect("Failed to start file");
...
fn test_orl_archive_parsing() {
    ...
    assert!(r4.archive_context.is_none(), "Without entry param, should not have archive context");
