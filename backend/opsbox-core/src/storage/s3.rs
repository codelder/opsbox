...
pub fn get_or_create_s3_client(url: &str, access_key: &str, secret_key: &str) -> Result<Arc<S3Client>, S3Error> {
  ...
  // 对于 localhost / IP 形式的 endpoint，强制 path-style。
  // 原因：virtual-host 风格会把请求变成 `bucket.<host>`，当 host 是 IP（如 127.0.0.1）时往往无法解析，
  // 会导致 S3 请求失败（例如 E2E 的 mock S3 场景）。
  let force_path_style = should_force_path_style(url);
  info!("S3 客户端参数: force_path_style={}", force_path_style);
...
fn should_force_path_style(endpoint_url: &str) -> bool {
  let Ok(url) = Url::parse(endpoint_url) else {
    // 解析失败时不强制（后续请求会报更明确错误）
    return false;
  };
  ...
  // IP 地址强制 path-style（避免 bucket.<ip> 域名无法解析）
