...
impl LogConfigRepository {
    ...
    pub async fn update_level(&self, component: &str, level: LogLevel) -> Result<(), LogError> {
        ...
        sqlx::query(
            "UPDATE log_config SET level = ?, updated_at = ? WHERE component = ?",
        )
        .bind(&level_str)
        .bind(now)
        .bind(component)
    ...
    pub async fn update_retention(
        ...
        sqlx::query(
            "UPDATE log_config SET retention_count = ?, updated_at = ? WHERE component = ?",
        )
        .bind(count as i64)
        .bind(now)
        .bind(component)
    ...
    async fn ensure_config_exists(&self, component: &str) -> Result<(), LogError> {
        let exists = sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*) FROM log_config WHERE component = ?",
        )
        .bind(component)
        .fetch_one(&self.pool)
        .await
        ...
        if exists == 0 {
            ...
            sqlx::query(
                "INSERT INTO log_config (component, level, retention_count, updated_at) VALUES (?, 'info', 7, ?)",
            )
            .bind(component)
            .bind(now)
            .execute(&self.pool)
    ...
    pub async fn get_response(
        ...
        component: &str,
        log_dir: PathBuf,
    ) -> Result<LogConfigResponse, LogError> {
...
mod tests {
    ...
    async fn test_update_level() {
        ...
        // 更新日志级别
        repo.update_level("server", LogLevel::Debug)
            .await
            .unwrap();
