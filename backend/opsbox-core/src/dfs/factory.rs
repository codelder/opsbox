...
use super::{
  endpoint::{Endpoint, Location, StorageBackend},
  filesystem::{FsError, OpbxFileSystem},
  impls::{AgentClient, AgentProxyFS, LocalFileSystem, S3Config as ImplS3Config, S3Storage},
};
...
pub enum FsConfig {
  ...
  /// S3 对象存储配置
  S3 { configs: HashMap<String, ImplS3Config> },
...
impl Clone for FsConfig {
  fn clone(&self) -> Self {
    match self {
      ...
      FsConfig::S3 { configs } => FsConfig::S3 {
...
impl std::fmt::Debug for FsConfig {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match self {
      ...
      FsConfig::S3 { configs } => f.debug_tuple("S3").field(&configs.keys().collect::<Vec<_>>()).finish(),
  ...
  match (&endpoint.backend, config) {
    ...
    (_, FsConfig::Agent { client_factory }) => {
      if let Location::Remote { host, port } = &endpoint.location {
        let agent_client = client_factory(host, *port).map_err(FsError::Agent)?;
        let fs = AgentProxyFS::new(agent_client);
        Ok(Box::new(fs))
      } else {
        Err(FsError::InvalidConfig(
          "Agent config requires Remote location".to_string(),
        ))
      }
    }
...
mod tests {
  ...
  fn test_create_fs_agent() {
    ...
    let config = FsConfig::Agent {
      client_factory: Box::new(|host, port| AgentClient::new(host.to_string(), port).map_err(|e| e.to_string())),
    };
    let result = create_fs(&endpoint, &config);
    assert!(result.is_ok());
  }
