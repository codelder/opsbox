...
impl Resource {
  /// 创建新的资源
  pub fn new(endpoint: Endpoint, primary_path: ResourcePath, archive_context: Option<ArchiveContext>) -> Self {
    Self {
      endpoint,
      primary_path,
      archive_context,
...
mod tests {
  ...
  fn test_resource_local() {
    ...
    assert!(matches!(
      ...
      super::super::endpoint::Location::Local
    ));
  ...
  fn test_resource_local_archive() {
    ...
    assert!(resource.is_archive());
    assert_eq!(
      resource.archive_context.as_ref().unwrap().inner_path.to_string(),
      "inner/file.txt"
    );
  }
  ...
  fn test_resource_s3() {
    ...
    assert!(matches!(
      resource.endpoint.location,
      super::super::endpoint::Location::Cloud
    ));
  ...
  fn test_resource_agent() {
    ...
    assert!(matches!(
      resource.endpoint.location,
      super::super::endpoint::Location::Remote { .. }
    ));
