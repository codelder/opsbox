  ...
  F: OpbxFileSystem,
{
  ...
  async fn scan_all_entries(&self) -> Result<Vec<ArchiveEntry>, FsError> {
    match self.archive_type {
      ArchiveType::Unknown => Err(FsError::InvalidConfig("Unknown archive type".to_string())),
      ArchiveType::Tar => {
    ...
    R: tokio::io::AsyncRead + Unpin,
  {
    ...
    tracing::debug!(
      "collect_tar_entries: completed, collected {} entries in {:?}",
      count,
      total_elapsed
    );
  ...
  async fn collect_tar_gz_entries_blocking(&self) -> Result<Vec<ArchiveEntry>, FsError> {
    ...
    tokio::task::spawn_blocking(move || {
      ...
      let file = File::open(&path).map_err(|e| FsError::Io(io::Error::new(io::ErrorKind::NotFound, e.to_string())))?;
        ...
        .map_err(|e| FsError::InvalidConfig(format!("Failed to read TAR entries: {}", e)))?
      {
        ...
        if count % 5000 == 0 {
          let elapsed = start_loop.elapsed();
          tracing::debug!(
            "collect_tar_gz_entries_blocking: processed {} entries in {:?}",
            count,
            elapsed
          );
        }
      ...
      let total_elapsed = start.elapsed();
      tracing::debug!(
        "collect_tar_gz_entries_blocking: completed, collected {} entries in {:?}",
        count,
        total_elapsed
      );
  ...
  async fn collect_zip_entries(
    &self,
    reader: &async_zip::tokio::read::seek::ZipFileReader<BufReader<File>>,
  ) -> Result<Vec<ArchiveEntry>, FsError> {
  ...
  async fn metadata(&self, path: &ResourcePath) -> Result<FileMetadata, FsError> {
    ...
    match self.archive_type {
      ...
      ArchiveType::Zip => {
        ...
        // 1. 精确匹配文件
        if let Some(entry) = items.iter().find(|e| {
          e.filename()
            .as_str()
            .ok()
  ...
  async fn read_dir(&self, path: &ResourcePath) -> Result<Vec<DirEntry>, FsError> {
    ...
    match self.archive_type {
      ...
      ArchiveType::Gz => {
        // Gzip 文件只包含单个文件
        let path = self.archive_path()?;
        let file_name = path.file_stem().and_then(|s| s.to_str()).unwrap_or("content");
        ...
        if dir_path.is_empty() || dir_path == "/" {
          ...
          Err(FsError::NotFound(format!(
            "Gzip file has no subdirectories: {}",
            dir_path
          )))
        }
      }
  ...
  async fn open_read(&self, path: &ResourcePath) -> Result<Pin<Box<dyn tokio::io::AsyncRead + Send + Unpin>>, FsError> {
    ...
    match self.archive_type {
      ...
      ArchiveType::Zip => {
        ...
        if let Some(index) = reader.file().entries().iter().position(|e| {
          e.filename()
            .as_str()
            .ok()
            .and_then(|s| Self::normalize_path(std::path::Path::new(s)).ok())
  ...
  F: OpbxFileSystem + Send + Sync,
{
  ...
  async fn as_entry_stream(
    ...
    match self.archive_type {
      ArchiveType::Tar => {
        let stream = crate::fs::TarArchiveEntryStream::new_tar(
      ...
      ArchiveType::TarGz | ArchiveType::Tgz => {
        let stream = crate::fs::TarArchiveEntryStream::new_tar_gz(
          tokio::fs::File::open(&archive_path).await.map_err(FsError::Io)?,
          Some(archive_path.to_string_lossy().to_string()),
        )
        .await
      ...
      ArchiveType::Unknown => Err(FsError::InvalidConfig("Unknown archive type for search".to_string())),
    }
  }
  ...
  fn supports_streaming_search(&self) -> bool {
    matches!(
      self.archive_type,
      ArchiveType::Tar | ArchiveType::TarGz | ArchiveType::Tgz | ArchiveType::Gz
    )
  }
}
...
mod tests {
  ...
  async fn test_tar_metadata() {
    ...
    let archive_fs =
      ArchiveFileSystem::with_temp_file(local_fs, ArchiveType::Tar, tar_path, NamedTempFile::new().unwrap());
  ...
  async fn test_zip_metadata() {
    ...
    let local_fs = LocalFileSystem::new(temp_dir.path().to_path_buf()).unwrap();
    let archive_fs =
      ArchiveFileSystem::with_temp_file(local_fs, ArchiveType::Zip, zip_path, NamedTempFile::new().unwrap());
  ...
  async fn test_normalize_path() {
    assert_eq!(
      ArchiveFileSystem::<LocalFileSystem>::normalize_path(std::path::Path::new("./test.txt")).unwrap(),
      "test.txt"
    );
    assert_eq!(
      ArchiveFileSystem::<LocalFileSystem>::normalize_path(std::path::Path::new("/test.txt")).unwrap(),
    ...
    assert_eq!(
      ArchiveFileSystem::<LocalFileSystem>::normalize_path(std::path::Path::new("./dir/test.txt")).unwrap(),
      "dir/test.txt"
    );
    assert_eq!(
      ArchiveFileSystem::<LocalFileSystem>::normalize_path(std::path::Path::new("/dir/test.txt")).unwrap(),
  ...
  async fn test_tar_gz_metadata() {
    ...
    let local_fs = LocalFileSystem::new(temp_dir.path().to_path_buf()).unwrap();
    let archive_fs =
      ArchiveFileSystem::with_temp_file(local_fs, ArchiveType::TarGz, tar_gz_path, NamedTempFile::new().unwrap());
