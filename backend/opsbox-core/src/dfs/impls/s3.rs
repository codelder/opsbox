...
impl S3Config {
  /// 创建新的 S3 配置
  pub fn new(profile_name: String, endpoint: String, access_key: String, secret_key: String) -> Self {
    Self {
      profile_name,
      endpoint,
      access_key,
...
impl S3Storage {
  ...
  pub fn new(config: S3Config) -> Result<Self, FsError> {
    ...
    .join()
    .map_err(|_| FsError::InvalidConfig("Thread join failed".to_string()))?
  }
  ...
  pub async fn new_async(config: S3Config) -> Result<Self, FsError> {
    ...
    // 配置 credentials
    let creds = Credentials::new(config.access_key, config.secret_key, None, None, "opsbox-s3");
  ...
  fn object_to_metadata(obj: &Object) -> FileMetadata {
    ...
    let last_modified = obj.last_modified().and_then(|dt| {
      ...
      std::time::SystemTime::UNIX_EPOCH.checked_add(std::time::Duration::from_secs_f64(dt.as_secs_f64()))
    });
  ...
  async fn read_dir(&self, path: &ResourcePath) -> Result<Vec<DirEntry>, FsError> {
    ...
    if let Some(objects) = output.contents {
      for obj in objects {
        ...
        // 提取名称（相对于 prefix 的名称）
        let name = key.trim_start_matches(&prefix).trim_start_matches('/').to_string();
