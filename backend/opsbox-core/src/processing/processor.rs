...
use crate::fs::{EntryMeta, EntryStream, PrefixedReader};

use super::filter::PathFilter;
use super::preload::{PreloadResult, preload_entry};
...
pub struct EntryStreamProcessor<P: ContentProcessor> {
  ...
  /// 检测下游是否已停止接收（如通道已关闭）
  is_stopped_fn: Option<Arc<dyn Fn() -> bool + Send + Sync>>,
  /// 错误回调（用于上报归档处理等错误到前端）
...
impl<P: ContentProcessor + 'static> EntryStreamProcessor<P> {
  ...
  /// 设置下游停止检测函数
  ///
  /// 当回调返回 `true` 时，处理器将主动终止扫描任务
  /// 典型用法：检测 mpsc::Sender::is_closed()
  pub fn with_is_stopped_fn(mut self, f: impl Fn() -> bool + Send + Sync + 'static) -> Self {
    self.is_stopped_fn = Some(Arc::new(f));
    self
  }

  /// 设置错误回调
  ///
  /// 用于将归档处理错误等上报到前端（如发送 SearchEvent::Error）
  pub fn with_error_callback(mut self, f: impl Fn(String) + Send + Sync + 'static) -> Self {
  ...
  where
    F: Fn(ProcessedContent) -> Fut + Send + Sync + 'static,
    Fut: Future<Output = ()> + Send,
  {
    ...
    loop {
      ...
      // 检查下游通道是否已关闭
      if let Some(ref is_stopped) = self.is_stopped_fn
        && is_stopped()
      {
    ...
    match preload_entry(&mut reader, self.preload_threshold).await {
      Ok(PreloadResult::Complete(content)) => {
        ...
        let handle = tokio::spawn(async move {
          let mut mem_reader: Box<dyn AsyncRead + Send + Unpin> = Box::new(std::io::Cursor::new(content));
          ...
          .await
          {
            Ok(Ok(Some(mut result))) => {
              ...
              cb_clone(result).await;
            }
            Ok(Ok(None)) => {}
            Ok(Err(e)) => {
              let msg = format!("处理预读条目内容失败: {}", e);
              warn!("{}", msg);
      ...
      Ok(PreloadResult::Partial(prefix)) => {
        ...
        match tokio::time::timeout(
          content_timeout,
          proc_clone.process_content(path.clone(), &mut boxed_reader),
        )
        .await
        {
          Ok(Ok(Some(mut result))) => {
