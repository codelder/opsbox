...
static RELOAD_HANDLE: std::sync::OnceLock<Arc<opsbox_core::logging::ReloadHandle>> =
...
fn init_test_logging_once() -> Arc<opsbox_core::logging::ReloadHandle> {
    RELOAD_HANDLE
        .get_or_init(|| {
            ...
            let reload_handle =
                Arc::new(opsbox_core::logging::init(log_config).expect("初始化日志系统失败"));
...
fn create_test_router(config: Arc<opsbox_agent::AgentConfig>) -> axum::Router {
    ...
    axum::Router::new()
        ...
        .route(
            ...
            put(opsbox_agent::update_log_retention),
        )
