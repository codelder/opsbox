...
impl AgentExplorer {
    ...
    pub async fn list(
        ...
        // 安全检查：解析并验证路径
        let resolved_paths = resolve_directory_path(&self.config, path)
            .map_err(|e| AgentExplorerError::PathResolution(e))?;

        let resolved_path = &resolved_paths[0];

        // 检测是否是归档文件
        if let Some(archive_type) = self.lister.detect_archive_type(resolved_path).await {
            return self.lister.list_archive(resolved_path, archive_type, None)
                .await
                .map_err(|e| AgentExplorerError::FileOperation(e));
        }

        // 普通目录列表
        self.lister.list_local(resolved_path)
            .await
            .map_err(|e| AgentExplorerError::FileOperation(e))
    }
    ...
    async fn list_archive_entry(
        ...
        // 解析归档文件路径（使用文件路径解析逻辑）
        let resolved_paths = resolve_directory_path(&self.config, archive_path)
            .map_err(|e| AgentExplorerError::PathResolution(e))?;

        let resolved_path = &resolved_paths[0];

        // 检测归档类型
        let archive_type = self.lister.detect_archive_type(resolved_path).await
            .ok_or_else(|| AgentExplorerError::UnsupportedArchiveType(
                format!("Unknown archive type: {}", archive_path)
            ))?;

        // 列出归档内容
        self.lister.list_archive(resolved_path, archive_type, Some(inner_path))
            .await
            .map_err(|e| AgentExplorerError::FileOperation(e))
    }
    ...
    pub async fn download(
        ...
        // 下载文件
        let (name, size, reader) = self.lister.download_local(resolved_path)
            .await
            .map_err(|e| AgentExplorerError::FileOperation(e))?;
    ...
    async fn download_archive_entry(
        ...
        // 下载归档内文件
        self.lister.download_archive_entry(resolved_path, inner_path, archive_type)
            .await
            .map_err(|e| AgentExplorerError::FileOperation(e))
    }
...
mod tests {
    ...
    async fn test_download_file() {
        ...
        let (name, size, mut reader) = explorer.download(&canon_file.to_string_lossy(), None)
            .await
            .unwrap();
    ...
    fn test_guess_mime_type() {
        assert_eq!(AgentExplorer::guess_mime_type("test.txt"), Some("text/plain".to_string()));
        assert_eq!(AgentExplorer::guess_mime_type("test.json"), Some("application/json".to_string()));
        assert_eq!(AgentExplorer::guess_mime_type("test.tar.gz"), Some("application/gzip".to_string()));
    }
}
