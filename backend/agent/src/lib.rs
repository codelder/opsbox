    ...
    impl axum::response::IntoResponse for ApiError {
        fn into_response(self) -> axum::response::Response {
            ...
            let (status, message) = match self {
                ApiError::InvalidLevel(msg) => {
                    (StatusCode::BAD_REQUEST, format!("无效的日志级别: {}", msg))
                }
...
) -> Result<Json<LogConfigResponse>, ApiError> {
...
pub async fn update_log_level(
    ...
    let level =
        LogLevel::from_str(&req.level).map_err(|e| ApiError::InvalidLevel(e.to_string()))?;
