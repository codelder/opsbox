...
目录结构
- server/api-gateway: 后端入口，监听 127.0.0.1:4000，挂载 /api/v1/logseek/*
- server/logseek: 日志检索工具库，导出 router()，提供 /stream
...
- 前端代理默认指向 http://127.0.0.1:4000/api/v1/logseek/stream

AI 查询串生成（本地 Ollama）
- 新增后端接口：POST /api/v1/logseek/nl2q
  - 请求体：{ "nl": "自然语言需求" }
  - 响应体：{ "q": "生成的查询字符串" }
